...
import { searchPackageInfo } from './web-search';
...
export async function verifyPackage(
  ...
  const webInfo = await searchPackageInfo(packageName);
...
Return ONLY valid JSON with your assessment.`;
  ...
  try {
    ...
    const result = JSON.parse(jsonMatch[0]) as PackageVerification;
...
export async function verifyPackages(
  packages: Array<{ name: string; purpose: string }>,
  apiKey: string,
