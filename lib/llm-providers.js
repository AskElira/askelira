...
exports.testLLMProvider = testLLMProvider;
 ...
 */
async function callOllama(config, systemPrompt, userMessage, maxTokens) {
 ...
 */
async function callLLM(config, systemPrompt, userMessage, maxTokens = 4096) {
 ...
 */
async function testLLMProvider(config) {
    try {
        const response = await callLLM(config, 'You are a helpful assistant.', 'Say "OK" if you can hear me.', 50);
