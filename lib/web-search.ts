 ...
 */
export async function searchAPIDocumentation(apiName: string, provider?: WebSearchOptions['provider']): Promise<SearchResult[]> {
  return await webSearch({
    query: `${apiName} API documentation tutorial examples 2026`,
    count: 5,
    freshness: 'month',
    provider,
  });
 ...
 */
export async function checkServiceStatus(serviceName: string): Promise<{
  active: boolean;
  deprecated: boolean;
  alternativesFound: string[];
}> {
  const results = await webSearch({
    query: `${serviceName} deprecated shutdown discontinued 2026`,
    count: 5,
    freshness: 'month',
  });
  ...
  const deprecated = text.includes('deprecated') || text.includes('shutdown') || text.includes('discontinued');
  const active = !deprecated && results.some(r => r.snippet.includes('documentation') || r.snippet.includes('getting started'));

  // Look for alternatives mentioned
  const alternatives: string[] = [];
  const altMatch = text.match(/alternative[s]?:?\s*([a-z0-9\s,]+)/i);
  if (altMatch) {
    alternatives.push(...altMatch[1].split(/[,\s]+/).filter(a => a.length > 2));
  }

  return {
    active,
    deprecated,
    alternativesFound: alternatives.slice(0, 3),
  };
}
